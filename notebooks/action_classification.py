...
def get_video(path="sample_video.mp4"):
    vr = VideoReader(path)
...
def get_video(path="sample_video.mp4"):
    vr = VideoReader(path)
    total_frames = len(vr) # Get the total number of frames in the video
    # choosing some frames here, you can define more complex sampling strategy
    # Ensure frame_idx does not go out of bounds
    frame_idx = np.arange(0, min(128, total_frames), 2)
    video = vr.get_batch(frame_idx).asnumpy()
    return video


def forward_vjepa_video(model_hf, model_pt, hf_transform, pt_transform, path):
...
model_pt, pt_video_transform = None, None
out_patch_features_hf, out_patch_features_pt = forward_vjepa_video(
    model_hf, model_pt, hf_transform, pt_video_transform, sample_video_path
)
...
classifier = (
    AttentiveClassifier(embed_dim=model_hf.config.hidden_size, num_heads=16, depth=4, num_classes=174).cuda().eval()
)
